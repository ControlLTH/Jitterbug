...
for s = 1:length(N.systems) % Check and fix interconnections
	...
	Q = Q + xtou'*N.systems{s}.Q*xtou; % Combined cost of state, outputs and inputs
	N.systems{s}.B = cellB;
	N.systems{s}.D = cellD;
end
	...
	if (N.systems{s}.type == 1) % Continuous
		...
		for t = 1:length(insys)
			if (insys(t) < 0)
				...
				error(['Continuous systems cannot use state as input']);
			end
			if (insys(t) == 0)
				% NULL input system
			else
				in = idtoindex(insys(t));
				if (isempty(N.systems{s}.impulse)) % Avoid ZOH inputs for impulse systems
					if (N.systems{in}.type == 1) % Continuous input
						A(ind(1,s):ind(2,s),(ind(1,in):ind(2,in))) = N.systems{s}.B{t}*N.systems{in}.C;
					else % Discrete input (use output-state)
						A(ind(1,s):ind(2,s),(ind(1,in)+size(N.systems{in}.A,1):ind(2,in))) = N.systems{s}.B{t};
					end
				else
					% inpulse input: do not hold the ctrl signal, so let the A-matrix be zero
				end
			end
		end
	end
	...
	if N.systems{s}.type == 1 && N.systems{s}.L > 0
		...
		for t = 1:length(insys)
			if (insys(t) == 0)
				...
				if (N.systems{in}.type == 1) % Continuous input
                    warning(['Continuous system output connected to continuous system with '...
                            'input delay: The signal will be zero-order held with time-grain '...
                            'dt, and the result will only be approximate.']);
...
for n = 1:length(N.nodes)
	...
	for s = 1:length(N.systems)
		...
		if (N.systems{s}.type == 1 && ~isempty(N.systems{s}.impulse))
			insys = N.systems{s}.inputid;
			if length(insys) > 1
